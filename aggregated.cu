...
__global__ void case3_mixed_kernel(
    ...
    half* __restrict__ global_sink) 
{
    ...
    // 核心逻辑：同一个 SP 内部 4 个 Warp 跑 4 个不同 i
    // 使得 SP 调度器总能找到一个处于不同依赖阶段的 Warp 进行发射
    int i_base = (warp_id % 4) * 2; 
...
int main() {
    ...
    printf("Launching Case 3 (Mixed): %d blocks, %d iterations...\n", grid.x, ITERATIONS);
    case3_mixed_kernel<<<grid, block, shmem_size>>>(d_weight, d_zero, d_scale, d_global_sink);
